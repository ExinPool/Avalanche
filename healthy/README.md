# Avalanche Node Healthy Monitor

> Avalanche node healthy monitor tools.
...
$ sudo git clone https://github.com/ExinPool/Avalanche
```
...
SERVICE=Avalanche
HEALTHY_VALUE=true
LOG_FILE=avalanche_healthy.log
...
``` bash
# Avalanche node healthy monitor
*/5 * * * * cd /data/monitor/exinpool/Avalanche/healthy && bash avalanche_healthy_lark.sh >> avalanche_healthy.log &
```

The crontab will run every minute then you can check the log in the `avalanche_healthy.log`.

## Features

- Monitor Avalanche node healthy
