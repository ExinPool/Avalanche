# Avalanche Node Process Monitor

> Avalanche node process monitor tools.
...
PROCESS=9650
PROCESS_NUM=1
LOG_FILE=avalanche_process.log
...
``` bash
# Avalanche node process monitor
*/5 * * * * cd /data/monitor/exinpool/Avalanche/process && bash avalanche_process_lark.sh >> avalanche_process.log &
```

The crontab will run every minute then you can check the log in the `avalanche_process.log`.

## Features

- Monitor Avalanche node process
